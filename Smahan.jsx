  ...
  const fetchProjects = async () => {
    try {
      ...
      if (currentSelectedId) {
        query = query.or(`id.eq.${currentSelectedId},project_id.eq.${currentSelectedId}`);
      }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center mb-24">
          ...
          <p className="mt-5 text-stone-400 leading-8 max-w-lg">
            استعرض نماذج من أعمالنا التي صغناها بعناية لتناسب أهداف علامتك التجارية.
          </p>
                ...
                <div className="absolute bottom-0 w-full p-6 md:p-8 z-20 flex flex-col justify-end">
                  <div className="transform transition-transform duration-500 group-hover:-translate-y-2">
                    ...
                    <div className="pt-2 border-t border-white/[0.08]">
                      <Link
                        to={`/projects/${project.project_id || project.id}?category=${encodeURIComponent(
                          project.category || ""
                        )}`}
                        onClick={() => {
                          localStorage.setItem("selectedProjectId", project.project_id || project.id);
                        }}
